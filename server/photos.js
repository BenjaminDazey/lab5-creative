...
router.post("/", auth.verifyToken, User.verify, upload.single('photo'), async (req, res) => {
  ...
  router.delete('/:id', async (req, res) => {
    try {
      await Photo.deleteOne({_id:mongoose.Types.ObjectId(req.params.id)});
  ...
  router.put('/:id', async (req, res) => {
    try {
      let bob = await Photo.findOne({_id:mongoose.Types.ObjectId(req.body.id)});
      bob.description = req.body.description;
      bob.save();
      res.sendStatus(200);
    } catch (error) {
      console.log(error);
